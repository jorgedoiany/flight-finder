...
const ComparisonRow: React.FC<{
  ...
  <TableRow>
    ...
    {flights.map((flight, index) => (
      ...
      >
        <ComparisonValue flight={flight} field={label.toLowerCase().replace(' ', '')} />
      </TableCell>
    ))}
  </TableRow>
)
      ...
      return (
        <Box textAlign="center">
          <Chip 
            ...
            size="small"
            color={flight.class === 'first' ? 'primary' : flight.class === 'business' ? 'secondary' : 'default'}
          />
        </Box>
      )
    case 'aircraft':
        ...
        <Stack spacing={2}>
          ...
          <Box textAlign="center">
            <Typography 
              ...
              fontWeight="bold"
              sx={{ 
                background: isBest?.price ? 'linear-gradient(45deg, #4caf50 30%, #81c784 90%)' : undefined,
          ...
          <Stack spacing={1}>
            <Box>
              ...
              <Typography variant="body2" color="text.secondary">
                {flight.departure.airport.code} - {flight.departure.airport.name}
              </Typography>
            </Box>
...
export const FlightComparison: React.FC<FlightComparisonProps> = ({ open, onClose }) => {
  const { comparison, removeFromComparison, clearComparison } = useFlightStore()
  ...
  const bestOptions = React.useMemo(() => {
    ...
    const prices = flights.map(f => typeof f.price === 'object' ? f.price.raw || 0 : f.price)
      ...
      <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Box textAlign="center" py={4}>
            ...
            <Typography variant="body2" color="text.secondary">
              Add flights to your comparison by clicking the compare button on flight results
            </Typography>
          </Box>
        </DialogContent>
        <DialogActions>
  ...
  return (
    ...
    >
      <DialogTitle sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <Typography>
          Compare Flights ({flights.length}/3)
        </Typography>
        <Box>
          <Button 
            size="small" 
        ...
        {viewMode === 'cards' ? (
          ...
          <TableContainer component={Paper} sx={{ maxHeight: '60vh' }}>
            <Table stickyHeader>
              ...
              <TableBody>
                <ComparisonRow label="Price" icon={<MoneyIcon fontSize="small" />} flights={flights} />
                <ComparisonRow label="Duration" icon={<ScheduleIcon fontSize="small" />} flights={flights} />
                <ComparisonRow label="Stops" icon={<StopsIcon fontSize="small" />} flights={flights} />
                <ComparisonRow label="Departure Time" icon={<ScheduleIcon fontSize="small" />} flights={flights} />
                <ComparisonRow label="Arrival Time" icon={<ScheduleIcon fontSize="small" />} flights={flights} />
                <ComparisonRow label="Airline" icon={<FlightIcon fontSize="small" />} flights={flights} />
                <ComparisonRow label="Class" icon={<ClassIcon fontSize="small" />} flights={flights} />
